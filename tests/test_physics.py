...
def _reference_score(cluster_ids, weights, lc_cp_idx, n_cp, reco_id):
    ...
    for j in range(n_cp):
        in_j = cp_for_lc == j
        # LCs in CP j contribute 0; LCs outside contribute e_k^2
...
def test_perfect_purity():
    ...
    assert result[0]["matched_cp_idx"] == 0


def test_mixed_cluster_score():
    # 4 LCs split evenly between two CPs by energy: [2, 2] to CP0, [1, 1] to CP1.
    ...
    assert result[0]["matched_cp_idx"] == 0
    assert result[0]["n_lc"] == 4
    assert result[0]["reco_energy"] == pytest.approx(6.0)
...
def test_outliers_not_treated_as_cluster():
    ...
    assert len(result) == 1
    assert result[0]["reco_id"] == 0
    assert result[0]["n_lc"] == 3
...
def test_multiple_clusters_independent():
    ...
    assert len(result) == 2
    scores = {r["reco_id"]: r["score"] for r in result}
...
def test_matches_reference_formula():
    ...
    assert result[0]["score"] == pytest.approx(ref, abs=1e-9)
