...
This is the CLUE analogue of `tune_agglomerative.py`: instead of clustering the
raw (x, y, z) coordinates, we run each event through a trained GNN embedder to
produce 3-D embeddings and cluster *those* with CLUE, keeping energy as the CLUE
weight exactly as in the raw pipeline.

Embeddings are computed once per event (they do not depend on the CLUE
parameters), then MOPSO searches over the CLUE thresholds only. Because the
embedding dimensions are abstract and symmetric (L2-normalized onto a unit
sphere), the metric is plain euclidean with no per-dimension weight — there is
no privileged axis, so the raw pipeline's `z_scale` parameter is dropped.

Parameters tuned (defined in config under 'parameters'):
  dc, rhoc, do, dm  — CLUE distance/density thresholds

Objectives (3), aggregated over the first N events in data.dir (same as tune_clue):
...
Usage:
  ...
  caloembed-tune-clue-gnn --config configs/tune_clue_gnn.yaml --resume
"""
...
class _Event:
    coords: np.ndarray          # (N, 3) GNN embedding
    weights: np.ndarray
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Tune CLUE parameters with MOPSO on GNN embeddings — see configs/tune_clue_gnn.yaml",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--config",  required=True, help="Tune config YAML (e.g. configs/tune_clue_gnn.yaml)")
