...
def main(argv=None):
    ...
    for i, event in enumerate(events):
        ...
        rows.append({
            ...
            "n_truth_cp": event.n_truth_cp,
            "sum_cp_energy": float(np.sum(event.cp_energies)),
            "mean_cp_energy": float(np.mean(event.cp_energies)) if len(event.cp_energies) > 0 else 0.0,
            "n_reco": m["n_reco"],
        ...
        if (i + 1) % 500 == 0:
            ...
            print(f"  {i+1} events processed ({elapsed:.1f}s) — "
                  f"last: {result.n_clusters} clusters [{result.backend}]")
    ...
    print(f"Summary: efficiency={summary['mean_efficiency']:.3f}  "
