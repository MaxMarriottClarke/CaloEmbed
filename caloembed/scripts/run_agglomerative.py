"""Cluster GNN embeddings with agglomerative clustering at a fixed threshold.

Embeds each event with a trained model, clusters the embeddings, and computes
the same physics metrics as the CLUE pipeline. Outputs match the schema of
caloembed-run (objects/efficiency/events.parquet + summary.json) so the result
overlays as a labelled curve in caloembed-plot.

Usage:
  caloembed-run-agglo --run-dir results/training/edgeconv_infonce \
      --data /path/to/hdf5 --output results/dataframes/d5/gnn_agglo \
      --threshold 0.052
"""
...
from caloembed.clustering.agglomerative import run_agglomerative
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="GNN + agglomerative clustering pipeline")
    ...
    parser.add_argument("--threshold", type=float, required=True, help="Cosine distance threshold")
    ...
    embedder = GNNEmbedder(args.run_dir, device=args.device)
    print(f"Model: {args.run_dir}  |  device: {embedder.device}  |  out_dim: {embedder.out_dim}")
    print(f"Clustering: {args.metric} + {args.linkage}  |  threshold: {args.threshold}")

    obj_cols, eff_cols, event_rows = {}, {}, []
    times = []
    t_start = time.perf_counter()

    events = iter_hdf5_dir(dir_path=args.data, max_events=args.max_events, max_files=args.max_files)
    for i, event in enumerate(events):
        t0 = time.perf_counter()
        emb = embedder.embed(event)
        cluster_ids = run_agglomerative(emb, args.threshold, method=args.linkage, metric=args.metric)
        ...
        obj_cols.setdefault("backend",    []).extend(["gnn_agglo"]      * n_reco)
        ...
        eff_cols.setdefault("backend",    []).extend(["gnn_agglo"]      * n_cp)
        ...
        for pdgid in np.unique(event.cp_pdg_ids):
            ...
            event_rows.append(dict(
                ...
                clue_ms=elapsed_ms, backend="gnn_agglo",
